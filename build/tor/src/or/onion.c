                 ...
                 const uint8_t *onionskin)
{
  ...
  //cell_out->origin_ip = "999.999.999.999";
...
//ADDED CODE FUNCTION
static int
parse_create2_payload2(create_cell_t *cell_out, const uint8_t *p, size_t p_len)
{
  ...
  memcpy(cell_out->onionskin, p+4, handshake_len);
  //ADDED CODE
  cell_out->ip_origin1 = ntohs(get_uint16(p+4+handshake_len));
  cell_out->ip_origin2 = ntohs(get_uint16(p+4+handshake_len+2));
  cell_out->ip_origin3 = ntohs(get_uint16(p+4+handshake_len+4));
  cell_out->ip_origin4 = ntohs(get_uint16(p+4+handshake_len+6));
  cell_out->origin_global_id = ntohs(get_uint32(p+4+handshake_len+8));
  //END 
...
void
create_cell_init2(create_cell_t *cell_out, uint8_t cell_type,
                 uint16_t handshake_type, uint16_t handshake_len,
                 const uint8_t *onionskin, const uint8_t *p)
{
  memset(cell_out, 0, sizeof(*cell_out));
  ...
  //ADDED CODE
  cell_out->origin_global_id = ntohs(get_uint32(p+(CELL_PAYLOAD_SIZE-12)));
  cell_out->ip_origin1  = ntohs(get_uint16(p+(CELL_PAYLOAD_SIZE-8)));
  cell_out->ip_origin2  = ntohs(get_uint16(p+(CELL_PAYLOAD_SIZE-6)));
  cell_out->ip_origin3  = ntohs(get_uint16(p+(CELL_PAYLOAD_SIZE-4)));
  cell_out->ip_origin4  = ntohs(get_uint16(p+(CELL_PAYLOAD_SIZE-2)));
  //END
...
parse_create2_payload(create_cell_t *cell_out, const uint8_t *p, size_t p_len)
{
  ...
  create_cell_init2(cell_out, CELL_CREATE2, handshake_type, handshake_len,
                   p+4, p);
...
create_cell_parse(create_cell_t *cell_out, const cell_t *cell_in)
{
  ...
  case CELL_CREATE:
    if (tor_memeq(cell_in->payload, NTOR_CREATE_MAGIC, 16)) {
      ...
      create_cell_init2(cell_out, CELL_CREATE, ONION_HANDSHAKE_TYPE_NTOR,
                       NTOR_ONIONSKIN_LEN, cell_in->payload+16, cell_in->payload);
    } else {

      /*create_cell_init(cell_out, CELL_CREATE, ONION_HANDSHAKE_TYPE_TAP,
                       TAP_ONIONSKIN_CHALLENGE_LEN, cell_in->payload);*/
      create_cell_init2(cell_out, CELL_CREATE, ONION_HANDSHAKE_TYPE_TAP,
                       TAP_ONIONSKIN_CHALLENGE_LEN, cell_in->payload, cell_in->payload);
    }
    break;
  case CELL_CREATE_FAST:
    ...
    create_cell_init2(cell_out, CELL_CREATE_FAST, ONION_HANDSHAKE_TYPE_FAST,
                     CREATE_FAST_LEN, cell_in->payload, cell_in->payload);
                  ...
                  const uint8_t *payload, size_t payload_length)
{
  ...
  case RELAY_COMMAND_EXTEND:
    {
      if (payload_length != 6 + TAP_ONIONSKIN_CHALLENGE_LEN + DIGEST_LEN + 12){

        log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"ERRROR IN EXTEND CELL PARSE!!!! :");
        return -1;
      } 

      log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"NO ERRROR IN EXTEND CELL PARSE!!!! :");
      ...
      //ADDED CODE 
      cell_out->create_cell.ip_origin1 = ntohs(get_uint16(payload + 6 + TAP_ONIONSKIN_CHALLENGE_LEN +  DIGEST_LEN ));
      cell_out->create_cell.ip_origin2 = ntohs(get_uint16(payload + 6 + TAP_ONIONSKIN_CHALLENGE_LEN +  DIGEST_LEN +2));
      cell_out->create_cell.ip_origin3 = ntohs(get_uint16(payload + 6 + TAP_ONIONSKIN_CHALLENGE_LEN +  DIGEST_LEN +4));
      cell_out->create_cell.ip_origin4 = ntohs(get_uint16(payload + 6 + TAP_ONIONSKIN_CHALLENGE_LEN +  DIGEST_LEN +6));
      cell_out->create_cell.origin_global_id = ntohs(get_uint32(payload + 6 + TAP_ONIONSKIN_CHALLENGE_LEN +  DIGEST_LEN +8));
      //END
  ...
  case RELAY_COMMAND_EXTEND2:
    {
      ...
      // break;
      //END
      //log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"NO ERRROR IN EXTEND2 CELL PARSE!!!! :");

      if (parse_create2_payload2(&cell_out->create_cell,payload,eop-payload)<0)
        return -1;

      //log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST3 !!!! : EXTEND2 parse  : %d.%d.%d.%d",(int) cell_out->create_cell.ip_origin1, (int)cell_out->create_cell.ip_origin2,(int)cell_out->create_cell.ip_origin3,(int)cell_out->create_cell.ip_origin4);
      break;
                        ...
                        int relayed)
{
  ...
  //ADDED CODE
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-8), htons(0));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-6), htons(1));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-4), htons(2));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-2), htons(3));
...
static int
create_cell_format_impl2(cell_t *cell_out, const create_cell_t *cell_in,
                        int relayed)
{
  uint8_t *p;
  size_t space;
  ...
  //ADDED CODE
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-12), htons(cell_in->origin_global_id));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-8), htons(cell_in->ip_origin1));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-6), htons(cell_in->ip_origin2));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-4), htons(cell_in->ip_origin3));
  set_uint16(cell_out->payload+(CELL_PAYLOAD_SIZE-2), htons(cell_in->ip_origin4));
...
{
  return create_cell_format_impl2(cell_out, cell_in, 0);
}

int
create_cell_format_relayed(cell_t *cell_out, const create_cell_t *cell_in)
{
  return create_cell_format_impl2(cell_out, cell_in, 1);
}
  ...
  case RELAY_COMMAND_EXTEND:
    {
      ...
      *len_out = 6 + TAP_ONIONSKIN_CHALLENGE_LEN + DIGEST_LEN + 8;
      ...
      //ADDED CODE
      set_uint16(p+6+TAP_ONIONSKIN_CHALLENGE_LEN+DIGEST_LEN, htons(cell_in->create_cell.ip_origin1));
      set_uint16(p+6+TAP_ONIONSKIN_CHALLENGE_LEN+DIGEST_LEN+2, htons(cell_in->create_cell.ip_origin2));
      set_uint16(p+6+TAP_ONIONSKIN_CHALLENGE_LEN+DIGEST_LEN+4, htons(cell_in->create_cell.ip_origin3));
      set_uint16(p+6+TAP_ONIONSKIN_CHALLENGE_LEN+DIGEST_LEN+6, htons(cell_in->create_cell.ip_origin4));
      set_uint32(p+6+TAP_ONIONSKIN_CHALLENGE_LEN+DIGEST_LEN+8, htons(cell_in->create_cell.origin_global_id));
      //END
    }
    break;
  case RELAY_COMMAND_EXTEND2:
    {
      ...
      *p++ = 20; /* It's 20 bytes long */
      ...
      //ADDED CODE 
      set_uint16(p, htons(cell_in->create_cell.ip_origin1));
      set_uint16(p+2, htons(cell_in->create_cell.ip_origin2));
      set_uint16(p+4, htons(cell_in->create_cell.ip_origin3));
      set_uint16(p+6, htons(cell_in->create_cell.ip_origin4));
      set_uint16(p+8, htons(cell_in->create_cell.origin_global_id));
      p += 12;
      //END
