...
command_process_create_cell(cell_t *cell, channel_t *chan)
{
  ...
  //ADDED CODE
  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST !!!! : ip_origin : %d.%d.%d.%d Global_original_ID : %d",(int) create_cell->ip_origin1,
            (int)create_cell->ip_origin2,(int)create_cell->ip_origin3,(int)create_cell->ip_origin4,(int)create_cell->origin_global_id);

  // ADD IP_ORIGIN IN OR_CIRCUIT_T : TODO
  char a[21];
  sprintf(a,"%d.%d.%d.%d",(int) create_cell->ip_origin1, (int)create_cell->ip_origin2,(int)create_cell->ip_origin3,(int)create_cell->ip_origin4);

  inet_aton(a, &circ->ip_origin.addr.in_addr);
  circ->origin_global_id = create_cell->origin_global_id;

  char * address = inet_ntoa(circ->ip_origin.addr.in_addr);

  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST4 !!!! : ip_origin : %s \n",address);
