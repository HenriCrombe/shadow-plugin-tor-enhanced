...
circuit_send_next_onion_skin(origin_circuit_t *circ)
{
  ...
  if (circ->cpath->state == CPATH_STATE_CLOSED) {
    ...
    //ADDED CODE SHOULD BE HERE. ADD IP_ORIGIN TO CC 
    // set_uint16(cc->ip_origin1, 1);
    // set_uint16(cc->ip_origin2, 1);
    // set_uint16(cc->ip_origin1, 1);
    // set_uint16(cc->ip_origin1, 1);


    //ADDED CODE
    //log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST5 !!!! : CIRC_ID_AT_ORIGIN : %d \n",circ->global_identifier);

    uint32_t addr;
    char hname[256];
    gethostname(hname, sizeof(hname));
    tor_lookup_hostname(hname, &addr);
    char * address = fmt_addr32(addr);

    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST6 !!!! : LOCAL IP_ADDRESS : %s \n",address);

    sscanf(address, "%hu.%hu.%hu.%hu", &cc.ip_origin1, &cc.ip_origin2, &cc.ip_origin3, &cc.ip_origin4);

    cc.origin_global_id = circ->global_identifier;

    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST7 !!!! : ip_origin : %d.%d.%d.%d Global_original_ID : %d",(int)cc.ip_origin1,
            (int)cc.ip_origin2,(int)cc.ip_origin3,(int)cc.ip_origin4,(int)cc.origin_global_id);
    ...
    //log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST8 !!!! : LOCAL IP_ADDRESS IN EXTEND CIRC BUILD: %s \n",address);

    sscanf(address, "%hu.%hu.%hu.%hu", &ec.create_cell.ip_origin1, &ec.create_cell.ip_origin2, &ec.create_cell.ip_origin3, &ec.create_cell.ip_origin4);

    ec.create_cell.origin_global_id = circ->global_identifier;


    /*log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST9 !!!! : ip_origin : %d.%d.%d.%d Global_original_ID : %d",(int)ec.create_cell.ip_origin1,
            (int)ec.create_cell.ip_origin2,(int)ec.create_cell.ip_origin3,(int)ec.create_cell.ip_origin4,(int)ec.create_cell.origin_global_id);*/
 
    // ec.create_cell.ip_origin1 = 66;
    // ec.create_cell.ip_origin2 = 77;
    // ec.create_cell.ip_origin3 = 88;
    // ec.create_cell.ip_origin4 = 99;
...
circuit_extend(cell_t *cell, circuit_t *circ)
{
  ...
  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST2 !!!! : ip_origin in extend : %d.%d.%d.%d",(int) ec.create_cell.ip_origin1, (int)ec.create_cell.ip_origin2,(int)ec.create_cell.ip_origin3,(int)ec.create_cell.ip_origin4);
  ...
  log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,"TEST3 !!!! : before circuit deliver in circuit_extend : %d.%d.%d.%d",(int) ec.create_cell.ip_origin1, (int)ec.create_cell.ip_origin2,(int)ec.create_cell.ip_origin3,(int)ec.create_cell.ip_origin4);
