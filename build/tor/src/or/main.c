...
static void activate_padding(connection_t *conn);
...
static void 
activate_padding(connection_t *conn){
  //send_control_event(EVENT_CONN_BW,"PADDING WORKING !");
  //control_padding_conn(conn);
  if(conn->type == CONN_TYPE_OR){
    ...
    //control_event_padding_conn(conn);

    if( memcmp("11.0.0.1", address,8) == 0){
    //if( strcmp("11.0.0.1:",address) == 0){
      //control_event_padding_conn(conn);
      //  cell_t cell;
      // relay_header_t rh;
     ...
     //connection_write_to_buf(networkcell.body, cell_network_size, TO_CONN(or_conn));

      if( conn->state == OR_CONN_STATE_OPEN ){
        cell_t cell;
        memset(&cell, 1, sizeof(cell_t));
        cell.circ_id = 999;
        cell.command = CELL_PADDING;
        packed_cell_t networkcell;
        size_t cell_network_size = get_cell_network_size(or_conn->wide_circ_ids);
        cell_pack(&networkcell, &cell, or_conn->wide_circ_ids);
        //char * payload = "Hello\n";
        // int size = 10000;
        // memset(payload, 1, sizeof(size));

        //connection_write_to_buf(payload, sizeof(6), conn);

        //connection_write_to_buf(networkcell.body, cell_network_size, TO_CONN(or_conn));

        //bufferevent_write(conn->bufev, networkcell.body, cell_network_size);
        //if(conn->outbuf_flushlen == 0){
        //connection_or_write_cell_to_buf(&cell, or_conn);

        //write_to_buf((char *)networkcell.body, cell_network_size, conn->outbuf);
        //connection_start_writing(conn);
        //conn->outbuf_flushlen += cell_network_size;
        //conn->n_written_conn_bw += cell_network_size; 
