...
control_event_stream_bandwidth(edge_connection_t *edge_conn)
{
  ...
  if (EVENT_IS_INTERESTING(EVENT_STREAM_BANDWIDTH_USED)) {
    ...
    send_control_event(EVENT_STREAM_BANDWIDTH_USED,
                       "650 STREAM_BW "U64_FORMAT" With-CIRC-ID=%lu  R=%lu W=%lu\r\n",
...
control_event_conn_bandwidth(connection_t *conn)
{
  ...
  conn->timestamp_created;

  char buff[20];
  char buff_last_read[20];
  char buff_last_write[20];
  time_t now = conn->timestamp_created;

  time_t last_read = conn->timestamp_lastread;
  time_t last_write = conn->timestamp_lastwritten;

  strftime(buff, 20, "%H:%M:%S", localtime(&now));
  strftime(buff_last_read, 20, "%H:%M:%S", localtime(&last_read));
  strftime(buff_last_write, 20, "%H:%M:%S", localtime(&last_write));

  char * address = inet_ntoa(conn->addr.addr.in_addr);



  send_control_event(EVENT_CONN_BW,
                      "650 CONN_BW  ID="U64_FORMAT" REMOTE_ADDR=%s:%lu TYPE=%s "
                     "READ=%lu WRITTEN=%lu  CREATED_AT=%s LAST_READ=%s LAST_WRITE=%s \r\n",
                     U64_PRINTF_ARG(conn->global_identifier),
                     address,
                     (unsigned long)conn->port,
                     conn_type_str,
...
int 
control_event_padding_conn(connection_t *conn){

  char * address = inet_ntoa(conn->addr.addr.in_addr);
  or_connection_t *or_conn = TO_OR_CONN(conn);


  send_control_event(EVENT_INFO_MSG,
                     "PADDING WORKING ??...!! Client OP IP Address = %s:%lu ",address,
                     (unsigned long)conn->port);
  return 0;
}

int
control_event_custom(connection_t *conn){
  char * address = inet_ntoa(conn->addr.addr.in_addr);

  send_control_event(EVENT_INFO_MSG,
                     "CONNECTION OPROXY ! Client OP IP Address = %s:%lu    ",address,
                     (unsigned long)conn->port);
  return 0;
}
                  ...
                  cell_stats_t *cell_stats)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    smartlist_add_asprintf(event_parts, "CIRCUIT_ORIGINATING_HERE LOCAL_ID=%lu",
                 (unsigned long)ocirc->global_identifier);
    if(circ->timestamp_dirty == 0){
    smartlist_add_asprintf(event_parts,"CIRCUIT_IS_NOT_DIRTY");

    } else{
      smartlist_add_asprintf(event_parts,"CIRCUIT_HAS_BEEN_USED=%d",circ->timestamp_dirty);
    }
  } else if (TO_OR_CIRCUIT(circ)->p_chan) {
    or_circuit_t *or_circ = TO_OR_CIRCUIT(circ);

    //ADDED CODE
    char * address = inet_ntoa(or_circ->ip_origin.addr.in_addr);

    int created_at = circ->timestamp_began.tv_sec;



    if(or_circ->is_first_hop == 1){
      smartlist_add_asprintf(event_parts, "CIRCUIT_RELAY_FIRST_HOP CIRC_ID_AT_ORIGIN=%d CIRC_IP_ORIGIN=%s CREATED_AT=%d",or_circ->origin_global_id, address,created_at);
    } else{
      if(circ->n_chan == NULL){
        smartlist_add_asprintf(event_parts, "CIRCUIT_RELAY_LAST_HOP CIRC_ID_AT_ORIGIN=%d CIRC_IP_ORIGIN=%s CREATED_AT=%d",or_circ->origin_global_id, address,created_at);
      }else{
        smartlist_add_asprintf(event_parts, "CIRCUIT_RELAY_MIDDLE CIRC_ID_AT_ORIGIN=%d CIRC_IP_ORIGIN=%s CREATED_AT=%d",or_circ->origin_global_id, address,created_at);
      }
    }
