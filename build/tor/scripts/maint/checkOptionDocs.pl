...
open(F, "/home/tor/shadow-plugin-tor/build/tor/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/home/tor/shadow-plugin-tor/build/tor/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/home/tor/shadow-plugin-tor/build/tor/doc/tor.1.txt") or die;
while (<F>) {
