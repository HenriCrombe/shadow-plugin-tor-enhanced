...
import os, sys, subprocess, argparse, time, shlex, shutil, random
...
# List of traces to replay per client-server pair
ltraces = []
...
def main():
    ...
    ap.add_argument('--firc', action="store", type=float, dest="firc", help="fraction F of IRC client connections", metavar='F', default=FIRC)
    ap.add_argument('--fssh', action="store", type=float, dest="fssh", help="fraction F of SSH client connections", metavar='F', default=FSSH)
    ...
    ap.add_argument('--traces', action="store", type=str, help="path to a folder containing traces to replay", metavar='TRACES', default=None)
    ap.add_argument('--ipsrc', action="store", type=str, help="ip of the source (when replaying traces)", metavar='IPSRC', default=None)
    ap.add_argument('--ipdst', action="store", type=str, help="ip of the destination (when replaying traces)", metavar='IPDST', default=None)
    ap.add_argument('--portsrc', action="store", type=str, help="port of the source (when replaying traces)", metavar='PORTSRC', default=None)
    ap.add_argument('--portdst', action="store", type=str, help="port of the destination (when replaying traces)", metavar='PORTDST', default=None)
    ...
    totalclientf = args.fweb + args.fbulk + args.firc + args.fssh
    if totalclientf != 1.0:
...
def traces_to_replay(traces_loc):
    traces = os.listdir(traces_loc)
    toReplay = ""
    for i in range(0, len(traces)):
        r = random.randint(0, len(traces)-1)
        toReplay += traces_loc + traces[r] + " "
    return toReplay
...
def generate(args):
    ...
    while i < args.nservers:
        ...
        if args.firc == 1.0:
            ...
            a.set("plugin", "pcap_replay")
            a.set("starttime", "1")
            ltraces.append(traces_to_replay(args.traces))
            a.set("arguments", "server server" + str(i) + " 80 " + args.ipsrc + " " + args.portsrc + " " + args.ipdst + " " + args.portdst + " 3500 " + ltraces[-1])
        if args.fssh == 1.0:
            a = etree.SubElement(e, "application")
            a.set("plugin", "pcap_replay")
            a.set("starttime", "1")
            ltraces.append(traces_to_replay(args.traces))
            a.set("arguments", "server server" + str(i) + " 80 " + args.ipsrc + " " + args.portsrc + " " + args.ipdst + " " + args.portdst + " 3500 " + ltraces[-1])
        if args.firc != 1.0 and args.fssh != 1.0:
            a = etree.SubElement(e, "application")
            a.set("plugin", "tgen")
            a.set("starttime", "1")
    ...
    while i <= args.nauths:
        ...
        addRelayToXML(root, starttime, torargs, None, None, None, name, download=6400, upload=6400)
    ...
    if args.nbridgeauths:
        ...
        while i <= args.nbridgeauths:
            ...
            addRelayToXML(root, starttime, torargs, None, None, None, name, bridgeauthority.download, bridgeauthority.upload, bridgeauthority.ip, bridgeauthority.code)
    ...
    for r in exitguards_nodes:
        ...
        addRelayToXML(root, starttime, torargs, None, None, None, name, r.download, r.upload, r.ip, r.code)
    ...
    nwebclients = int(args.fweb * args.nclients)
    nircclients = int(args.firc * args.nclients)
    nsshclients = int(args.fssh * args.nclients)
    ...
    secondsPerClient = 600.0 / (nbulkclients+nwebclients+nircclients+nsshclients+nperf50kclients+nperf1mclients+nperf5mclients)
    ...
    while i <= 1 and False: # TODO enable when torflow works
        ...
        addRelayToXML(root, starttime, torargs, None, None, None, name, download=1048576, upload=1048576, code=choice(clientCountryCodes), torflowworkers=max(int(args.nrelays/50), 1))
    ...
    while i <= nwebclients:
        ...
        addRelayToXML(root, starttime, torargs, tgenargs, None, None, name, code=choice(clientCountryCodes))
    ...
    i = 1
    while i <= nircclients:
        name = "ircclient{0}".format(i)
        starttime = "{0}".format(int(round(clientStartTime)))
        torargs = "{0} -f conf/tor.client.torrc".format(default_tor_args) # in bytes
        ircargs = "client-tor 9000 server" + str(i) + " 80 " + args.ipsrc + " " + args.portsrc + " " + args.ipdst + " " + args.portdst + " " + str(int(starttime)+870) + " " + ltraces[i-1]       
        addRelayToXML(root, starttime, torargs, None, ircargs, None, name, code=choice(clientCountryCodes))
    ...
    i = 1
    while i <= nsshclients:
        name = "sshclient{0}".format(i)
        starttime = "{0}".format(int(round(clientStartTime)))
        torargs = "{0} -f conf/tor.client.torrc".format(default_tor_args) # in bytes
        sshargs = "client-tor 9000 server" + str(i) + " 80 " + args.ipsrc + " " + args.portsrc + " " + args.ipdst + " " + args.portdst + " " + str(int(starttime)+870) + " " + ltraces[i-1]
        addRelayToXML(root, starttime, torargs, None, None, sshargs, name, code=choice(clientCountryCodes))
    ...
    with open("shadow.config.xml", 'wb') as fhosts:
        ...
        e = etree.Element("plugin")
        e.set("id", "pcap_replay")
        e.set("path", "{0}plugins/libshadow-plugin-pcap_replay.so".format(INSTALLPREFIX))
        root.insert(0, e)
...
def addRelayToXML(root, starttime, torargs, tgenargs, ircargs, sshargs, name, download=0, upload=0, ip=None, code=None, torflowworkers=1): # bandwidth in KiB
    # node
    e = etree.SubElement(root, "node")
    e.set("id", name)
...
#                a.set("arguments", "localhost {0} STREAM,CIRC,CIRC_MINOR,ORCONN,BW,STREAM_BW,CIRC_BW,CONN_BW,BUILDTIMEOUT_SET,CLIENTS_SEEN,GUARD,CELL_STATS,TB_EMPTY".format(controlport))
    ...
    if ircargs is not None:
        a = etree.SubElement(e, "application")
        a.set("plugin", "pcap_replay")
        a.set("starttime", "{0}".format(int(starttime)+300))
        a.set("arguments", ircargs)
    if sshargs is not None:
        a = etree.SubElement(e, "application")
        a.set("plugin", "pcap_replay")
        a.set("starttime", "{0}".format(int(starttime)+300))
        a.set("arguments", sshargs)
